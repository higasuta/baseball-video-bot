...
def generate_caption(title, desc):
    ...
    prompt = f"野球まとめ動画の管理人として。「{title}」「{desc}」から最高に熱いインスタ投稿を作れ。掟：1.標準語語り口調 2.見出し【朗報】等 3.全人物#タグ25個 4.URL不要。文章のみ。"
    try:
    ...
    if not video_data:
        ...
        if mlb_item:
            ...
            if is_test_mode or mlb_item['is_hot'] or ratio < 0.3:
                if is_test_mode: print("🛠 テストモードにつき比率無視")
                elif mlb_item['is_hot']: print("🔥 MLB大ニュースにつき優先")
                video_data = mlb_item
            else:
                print(f"📊 比率調整：MLB待機 (現在比率: {ratio:.2f})")
