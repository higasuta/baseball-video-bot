import sys
# 1行目からリアルタイムでログを出力
print("🚀 プレイボール速報・システム最終形態（NHK公式データ ＋ ログ透明化モード）起動...")
sys.stdout.flush()
...
def get_npb_video(history):
    ...
    try:
        ...
        print(f"  👉 NHK公式データから {len(clips)}件 の情報を取得。")
        ...
        print(f"  ⚠️ NHKルート失敗: {e}")
        return []

def get_mlb_video(history, is_test_mode):
    """MLB APIから日本人動画を探索（透明化ログ付き）"""
...
def main():
    ...
    if not candidates:
        print("😴 本当に新着がありません。history.txt が満杯の可能性があります。")
        return
    ...
    for video in candidates:
        ...
        # 全て yt-dlp で統一（NHK公式やMLB APIは安定）
