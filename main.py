import sys
# 1行目からリアルタイムでログを出力
print("🚀 プレイボール速報・システム最終形態（Bing/Livedoor 奪還ルート）起動...")
sys.stdout.flush()
...
def get_npb_video(history):
    """【新ルート】Bing RSS ＋ Livedoor ＋ goo ニュースからNPB動画を探索"""
    candidates = []
    ua = "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1"
    
    # 1. Bing News RSS (Microsoftの検索データ、XMLなのでブロックが極めて困難)
    print("🔍 NPBルートA (Bing News RSS) 探索中...")
    try:
        ...
        for item in root.findall('.//item')[:10]:
            ...
            if v_id not in history:
                if any(k in title for k in ["安打", "本塁打", "奪三振", "勝利", "得点"]):
                    print(f"  ✅ Bing RSSで発見: {title[:30]}")
                    candidates.append({"title": title.strip(), "url": v_url, "id": v_id, "type": "npb", "source": "BingNews"})
    except: pass

    # 2. Livedoor ニュース (野球ビデオカテゴリ - 独自の配信網)
    print("🔍 NPBルートB (Livedoorニュース) 探索中...")
    try:
        url = "https://news.livedoor.com/category/v/baseball/"
        res = requests.get(url, headers={"User-Agent": ua}, timeout=15)
        matches = re.findall(r'href="https://news\.livedoor\.com/article/detail/(\d+)/"', res.text)
        for v_id in list(dict.fromkeys(matches))[:3]:
            if v_id not in history:
                print(f"  ✅ Livedoorで発見: {v_id}")
                candidates.append({"title": "NPB注目シーン", "url": f"https://news.livedoor.com/article/detail/{v_id}/", "id": v_id, "type": "npb", "source": "LivedoorNews"})
    except: pass

    return candidates[0] if candidates else None

def get_mlb_video(history, is_test_mode):
    """MLB APIから日本人15名の活躍動画を探索"""
...
def analyze_video_with_ai(video_path, title, source_account, model_name):
    ...
    try:
        ...
        【キャプション執筆ルール】
        ・あなたは野球2chまとめのベテラン。皮肉と分析を交えた鋭い見出しから始めろ（ですます禁止）。
        ・[0:05] のようなタイムスタンプを本文に自然に組み込め。
        ・ラベル(START:、本文: 等)は絶対に書くな。
        ...
        # 【物理抹殺】ラベル文字を正規表現で徹底削除（AIっぽさを消す）
        clean_text = re.sub(r'(?i)(START|CAPTION|秒数|本文|開始|タイトル|見出し|概要|所感|解説|所見)[:：]\s*', '', res_text).strip()
...
def main():
    ...
    if video_data:
        ...
        # yt-dlp と curl の二段構えでダウンロード失敗を許さない
