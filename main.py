import sys
# リアルタイムログ出力設定
print("🚀 プレイボール速報・システム最終形態（不屈のリトライ ＋ MSN奪還モード）起動...")
sys.stdout.flush()
...
def find_working_ai_model():
    """環境で本当に使えるモデル名を自動特定"""
    try:
        available_models = [m.name for m in genai.list_models() if 'generateContent' in m.name]
...
def get_npb_candidates(history):
    """【執念】Bing, MSN, Livedoorから候補を「リスト」で返す"""
    candidates = []
    ua = "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1"
    
    # ルート1: Bing Video RSS (Microsoftインフラ)
    print("🔍 NPBルートA (Bing RSS) 探索中...")
    try:
        url = "https://www.bing.com/news/search?q=プロ野球+動画+OR+ハイライト&format=RSS&setlang=ja"
        ...
        for item in root.findall('.//item')[:10]:
            ...
            if v_id not in history:
                candidates.append({"title": item.find('title').text, "url": v_url, "id": v_id, "type": "npb", "source": "BingNews"})
    except: pass

    # ルート2: MSNスポーツ (Microsoft直営)
    print("🔍 NPBルートB (MSN Sports) 探索中...")
    try:
        url = "https://www.msn.com/ja-jp/sports/baseball"
        res = requests.get(url, headers={"User-Agent": ua}, timeout=15)
        v_ids = re.findall(r'\"id\":\"([a-zA-Z0-9_-]{10,})\"', res.text)
        for v_id in list(dict.fromkeys(v_ids))[:5]:
            if v_id not in history:
                candidates.append({"title": "NPB注目シーン", "url": f"https://www.msn.com/ja-jp/sports/baseball/video/{v_id}", "id": v_id, "type": "npb", "source": "MSN"})
    except: pass

    return candidates

def get_mlb_candidates(history, is_test_mode):
    """MLB日本人選手の動画候補をリストで返す"""
    print("🔍 MLB動画を探索中...")
    candidates = []
...
def analyze_video_with_ai(video_path, title, source_account, model_name):
    ...
    try:
        ...
        1. [秒数のみを1行目に]
        2. [本文を2行目以降に]
        ルール：野球2chまとめのベテラン解説。皮肉と分析を交えた鋭い見出し。ですます禁止。ラベル(START等)禁止。
        タイムスタンプ、タグ25個、引用：{source_account} を入れろ。
        """
