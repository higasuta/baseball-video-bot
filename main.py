...
def get_npb_video(history):
    """NPBスキャン：複数の回避ルートを使用"""
    # twitter.com と nitter.net (回避用ミラー) を併用
    sources = [
    ...
    for src in sources:
        ...
        print(f"🔍 @{user_name} を全力スキャン中...")
        try:
            # ブラウザ偽装をさらに強化
            cmd = [
                'yt-dlp', '--get-id', '--get-title', '--get-url', '--print', 'upload_date',
                '--playlist-end', '20', '--match-filter', "duration < 240 & !is_live",
                '--no-check-certificates', '--user-agent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36',
                '--quiet', '--no-warnings', src
            ]
            process = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
            lines = [l for l in process.stdout.split('\n') if l.strip()]
            
            print(f"  👉 取得データ: {len(lines)//4}件の候補")
...
def analyze_video_with_ai(video_path, title, source_account):
    ...
    try:
        ...
        prompt = (f"野球動画（タイトル：{title}）を解析せよ。\n1. 見どころ開始秒数を「START:秒」で。\n2. 2chまとめ解説動画風の熱いキャプションを作成。\n3. 最後に『引用：{source_account}』と記載。\nSTART:[秒]\nCAPTION:[内容]")
...
def main():
    ...
    if video_data:
        ...
        # 画質を上げ(crf 18)、Instagramが拒絶しないよう設定を強化
                ...
                if up_res.status_code == 200:
                    ...
                    if 'id' in post_res:
                        ...
                        for i in range(20):
                            ...
                            elif status == 'ERROR' or (status is None and 'error' in status_res):
                                print(f"❌ 処理失敗、次回の実行に期待します。"); return
                    else: print(f"❌ コンテナ作成失敗: {post_res}")
        except Exception as e: print(f"❌ システムエラー: {e}")
    else: print("😴 条件に合う未投稿動画が見つかりませんでした。")
