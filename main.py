import sys
# リアルタイムログ出力
print("🚀 プレイボール速報・システム最終形態（日本人MLB広域 ＋ 爆速完走モード）起動...")
sys.stdout.flush()
...
def get_npb_video(history):
    """スポーツナビから動画を探索（検閲を避けるためモバイル偽装）"""
    print("🔍 NPB探索 (スポーツナビ・ダイレクト)...")
    ...
    try:
        ...
        ids = re.findall(r'href="/video/player/(\d+)"', res.text)
        for v_id in list(dict.fromkeys(ids))[:5]:
            if v_id not in history:
                return {"title": "NPB注目シーン", "url": f"https://sports.yahoo.co.jp/video/player/{v_id}", "id": v_id, "type": "npb", "source": "SportsNavi"}
    except: pass
    return None

def get_mlb_video(history, is_test_mode):
    """日本人選手のプレー映像を幅広く取得"""
    print("🔍 MLB動画を探索中（日本人15名カバー）...")
    candidates = []
...
def analyze_video_with_ai(video_path, title, source_account, model_name):
    ...
    try:
        ...
        1. [秒数のみを1行目]
        2. [本文を2行目以降]
        ルール：野球2chまとめ風の鋭いナレーション（だ・である調）。ラベル(START:等)は絶対書くな。
        タイムスタンプを自然に入れろ。引用：{source_account} を最後に。タグ25個。
        """
...
def main():
    ...
    print(f"⚾️ 探索開始...")
    npb_data = get_npb_video(history)
    mlb_list = get_mlb_video(history, is_test_mode)
    
    # 候補リストを統合（NPB優先）
    candidates = ([npb_data] if npb_data else []) + mlb_list
    ...
    for video in candidates:
        # 比率制限: NPBがあればMLBはスキップ
        if not is_test_mode and video['type'] == 'mlb' and mlb_ratio > 0.25 and npb_data:
            continue
        ...
        # 【物理排除】HTMLファイル（偽動画）を即座にボツにするガード
        if os.path.exists(temp_input):
            with open(temp_input, 'rb') as f:
                header = f.read(100).decode(errors='ignore')
                if "<html" in header.lower() or os.path.getsize(temp_input) < 1000000:
                    print("❌ ダウンロード失敗（偽動画またはサイズ不足）。次を試します。")
                    continue
