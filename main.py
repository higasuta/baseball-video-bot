import sys
# リアルタイムログ出力
print("🚀 プレイボール速報・最終決戦（NPB 20ルート超・広域スキャン）起動...")
sys.stdout.flush()
...
def get_npb_video(history):
    """【執念】日本中のメディアサイト20箇所以上から動画を力技で探索"""
    candidates = []
    ua = "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1"
    
    # ターゲットサイト20選（新聞、TV、ネットメディア、RSS）
    targets = [
        "https://www.nikkansports.com/baseball/video/",
        "https://www.sponichi.co.jp/baseball/",
        "https://www.sanspo.com/baseball/",
        "https://hochi.news/baseball/",
        "https://baseballking.jp/video",
        "https://full-count.jp/category/npb/",
        "https://sportsbull.jp/category/baseball/",
        "https://www.daily.co.jp/baseball/",
        "https://news.yahoo.co.jp/categories/sports",
        "https://www3.nhk.or.jp/news/cat07.html",
        "https://news.tv-asahi.co.jp/news_sports/index.html",
        "https://news.ntv.co.jp/category/sports",
        "https://newsdig.tbs.co.jp/category/sports",
        "https://www.fnn.jp/category/スポーツ",
        "https://mainichi.jp/baseball/",
        "https://www.asahi.com/sports/baseball/",
        "https://www.yomiuri.co.jp/sports/baseball/",
        "https://dot.asahi.com/dot/sports/baseball/",
        "https://the-ans.jp/category/baseball/",
        "https://cocokara-next.com/category/athlete/baseball/",
        "https://www.tokyo-sports.co.jp/baseball/",
        "https://abema.tv/video/genre/sports"
    ]

    print(f"🔍 NPB全広域探索を開始（ターゲット数: {len(targets)}）...")

    for url in targets:
        site_name = url.split('/')[2]
        print(f"  👉 スキャン中: {site_name}")
        try:
            # yt-dlpの汎用抽出能力に賭ける
            cmd = ['yt-dlp', '--get-id', '--get-title', '--get-url', '--playlist-end', '3', '--no-check-certificates', '--user-agent', ua, '--quiet', url]
            output = subprocess.check_output(cmd, stderr=subprocess.DEVNULL, timeout=30).decode().split('\n')
            ...
            for i in range(0, len(output)-2, 3):
                ...
                if v_id and v_id not in history:
                    ...
                    print(f"    ✅ {site_name}で発見: {title}")
                    candidates.append({"title": title, "url": v_url, "id": v_id, "type": "npb", "source": site_name, "priority": 1})
        except: continue
    
    return candidates

def get_mlb_video(history, is_test_mode):
    """日本人選手に100%特化したMLBスキャン"""
...
def main():
    ...
    try:
        ...
        flash_model = next((pm for pm in priority_models if pm in model_names), model_names[0])
